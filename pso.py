...
class PSOTransform():
    def __init__(self, model, xx, filter, target_label, pN=30, dim=3, max_iter=100, num_samples=40, eta=0.6):
        self.model = model
        self.xx = xx
        self.filter = filter
    ...
    def function(self, param, save_flag=False):
        ...
        for _ in range(self.num_samples):
            ...
            if top1label == self.target_label:
                misclassified += 1
                quality = 1 - cal_face_quality(x.detach().cpu().numpy().astype(np.uint8))
    ...
    def iterator(self):
        ...
        for t in range(self.max_iter):
            for i in range(self.pN):
                ...
                if temp < self.p_fit[i]:
                    ...
                    if self.p_fit[i] < self.fit:
                        ...
                        self.error_rate = error_rate_loss
                        self.quality = avg_quality     
            self.V = self.w * self.V + self.c1 * self.r1 * (self.pbest - self.X) + self.c2 * self.r2 * (self.gbest - self.X)
            ...
            if ((abs(self.error_rate - 0.0)) < 0.1 and abs(self.quality - 0.0) < 0.5) or (t > 20 and abs(fitness[-1] - fitness[-11]) < 1e-8):
